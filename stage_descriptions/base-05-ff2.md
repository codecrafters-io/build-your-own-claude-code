...
```json
{
  ...
  "messages": [
    ...
    // The tool call resulted in a result, which must be appened to the 'messages' array
    // This is also appended to the 'messages' array
    {
      "role": "tool",
...
- Messages should be appended to the conversation history (e.g., user prompt → assistant response → tool result → assistant response → tool result → ...)
- Stop the loop when the response has `finish_reason: "stop"`, and there are no `tool_calls`. At this point, print the `content` to stdout and exit.

### Tests

- The tester will create multiple files, each with random contents.
- The tester will create two index files. These files will each contain the path of one randomly selected files from before.
...
- Your program should read the first two files.
