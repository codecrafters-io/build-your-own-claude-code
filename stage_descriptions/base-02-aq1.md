In this stage, you'll add support for advertising the `Read` tool to the LLM.
...
For this stage, you'll only need to advertise the **Read** tool, which allows the LLM to read file contents.
...
```bash
$ ./your_program.sh -p "<Prompt asking for the count of available tools in the request>"
1
```
...
- In this stage, you'll only need to advertise the availability of the `Read` tool in your request. We'll get to implementing the execution of the tool call in the next stage.

- The tester will only perform end-to-end tests. You are free to choose the name of the tool. For example, any of the following names are valid:
  - `Read`
  - `read`
  - `read_file`
  - `ReadFile`, etc.
