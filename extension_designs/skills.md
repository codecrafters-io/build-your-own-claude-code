...
| 7 | `mj2` | Run a skill in a subagent | `hard` | Honour `context: fork` by handing the body to a subagent — a second run of the agent loop over its own messages — and returning its answer to the main conversation | One request carries the body without the catalog; one carries both the catalog and the answer; the answer reaches stdout | `RequestRecorder`, `request_assertion` package, `context` frontmatter field | built |
...
**The extension has no CI coverage yet.** Phase 6 of the authoring procedure is incomplete: there is a reference implementation under `internal/test_helpers/scenarios/skills_stages/users_code_pass_all/`, and the unit tests pass, but no fixtures have been recorded, the skills slugs are absent from `internal/stages_test.go`, and `make test_flakiness` has never run. All of that needs a live model via `CODECRAFTERS_SECRET_OPENROUTER_API_KEY`. Until then the stage tests have never executed end to end, and "the unit tests pass" should not be read as more than it says.
...
- **A forked skill can't be stacked**, which killed the first design outright. Expansion stops at the first token that isn't an inline user-invocable skill, and a forked skill is one of those. The original shape invoked a normal skill and a forked one in a single prompt — `/main /forked` — and used the normal skill's body as the value that must be absent from the forked conversation. In Claude Code that prompt expands only `main` and hands it `/forked` as argument text, so the stage would have taught something false. Caught during phase 1 step 1 research, before the description was written. The marker is now a second skill's *name*, which reaches the model only through the catalog and so can't be stacked away.

**The stage exists because of observability, not difficulty.** Honouring `context: fork` doesn't change the answer, only where the work happened, so a correct submission and one that ignores the field print the same bytes. Verified rather than assumed: running the reference implementation against the same skill with the frontmatter field removed produces one request instead of two and identical stdout. Every other stage in the extension fails with output the learner can reproduce and diff by hand; this one can't, which is its real cost.
