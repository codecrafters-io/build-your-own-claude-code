...
(
  ...
  CLASSES_DIR="/tmp/codecrafters-build-claude-code-kotlin/classes"
  if [ -d "$LIBS_DIR" ] && [ "$(ls -A "$LIBS_DIR" 2>/dev/null)" ] && command -v kotlinc >/dev/null 2>&1; then
      ...
      mkdir -p "$LIBS_DIR"
      cp /tmp/codecrafters-build-claude-code-kotlin/install/app/lib/*.jar "$LIBS_DIR/"
  fi
)
...
if [ -d "$CLASSES_DIR" ]; then
    ...
    exec /tmp/codecrafters-build-claude-code-kotlin/install/app/bin/app "$@"
fi
